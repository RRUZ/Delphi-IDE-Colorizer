type
  TEditWindow=class(TDesktopForm)
  private
   FSupressTabChange   :Boolean;
   FSupressViewBarTabChange:Boolean;
   FCurrentHeight      :Integer;
   FClosing            :Boolean;
   FMousePos           :TPoint;
   FEditWindowId       :Integer;
   FEditorViews        :TObjectList;
   FDragCanvas         :TCanvas;
   FDragBitmap         :TBitmap;
   FNewDraggedIndex    :Integer;
   FCustomEventHandlers:TComponentList;
   FDefaultEditEventHandler:TDefaultEditEventHandler;
   FCurrentEditor      :TWinControl;
   FSelectedView       :TCustomEditView;
   FViewSelectUpdateCount:Integer;
   FWasDocked          :Boolean;
   FProcessingCmDialog :Boolean;
   FUpdating           :Integer;
   FRecreating         :Boolean;
   FRecreatingDesktop  :Boolean;
   FNewTabIndex        :Integer;
   ExpanderForm        :TForm;
   FViewMode           :TViewMode;
   FZoomMode           :TViewMode;
   FVisibleDockClients :TComponentList;
   SearchCloseButton   :TCustomGradientButton;
   FIncrementalSearch  :Boolean;
   FIncrSearchText     :string;
   FShiftState         :TShiftState;
   FSavedSearchFlags   :TEdSearchFlags;
   FAutoSave           :Boolean;
   FDeskSection        :string;
   FLocked             :Boolean;
   ...
   function GetEditorFrame(Index: Integer): TCustomFrame;
   function GetEditView(Index: Integer): TCustomEditView;
   function GetStructureContext(ViewIndex: Integer): IOTAStructureContext;
   function GetStructureType(ViewIndex: Integer): WideString;
  ...
  public
   CurEditView         :Integer;
   procedure SelectLastBuffer;
   procedure ShowDockableFormFrame(const FormName: string; const ACaption: string; AFrame: TFrame);
   procedure ActivateForm(SwitchToFirstTab: Boolean);
   procedure AddCustomEditEventHandler(EditorEventHandler: TCustomEditEventHandler);
   procedure BeginUpdate;
   function CreateDockableForm(const FormName: string): TDockableForm;
   procedure EndUpdate(NewIndex: Integer);
   procedure InsertView(AEditView: TCustomEditView; const ADefaultEditViewName: string);
   procedure NextEditView;
   procedure PrevEditView;
   procedure NextBufferView;
   procedure PrevBufferView;
   procedure RemoveView(AEditView: TCustomEditView);
   function SelectEditView(AEditView: TCustomEditView; const ADefaultEditViewName: string): Boolean;
   procedure SelectBuffer(Index: Integer; const ADefaultEditViewName: string);
   procedure SwitchToEditorView;
   procedure UpdateNames(EditView: TCustomEditView);
   procedure UpdateCaption;
   procedure ActivateModule;
   function EditAction(Action: TEditAction): Boolean;
   function GetEditState: TEditState;
   function GetCustomEditEventHandler(AControl: TWinControl): TCustomEditEventHandler;
   procedure SaveWindowState(Desktop: TCustomIniFile; isProject: Boolean);
   procedure LoadWindowState(Desktop: TCustomIniFile);
   procedure LoadDockData(DeskFile: TCustomIniFile);
   procedure LoadDesktopFormInstances(Desktop: TCustomIniFile);
   procedure LoadOwnedDockForms(Desktop: TCustomIniFile);
   procedure SaveOwnedDockForms(Desktop: TCustomIniFile; IsProject: Boolean);
   procedure OnNavigate(Sender: TObject; Forward: Boolean);
   procedure WindowActivate(SendViewNotification: Boolean);
   procedure UpdateViewInfo(CurEditView: TCustomEditView);
   procedure UpdateViewBarTabNames(CurEditView: TCustomEditView);
   procedure FocusCurrentEditor;
   procedure UpdateEditorFormName;
   procedure ModuleSelect;
   function ViewSelect(AEditView: TCustomEditView; Index: Integer; ForceChange: Boolean): TCustomEditView;
   function ViewIdentifierToIndex(const AViewIdentifier: string; out AViewIndex: Integer): Boolean;
   function ViewIndexToViewIdentifier(Index: Integer): string;
   function ActiveViewIdentifier: string;
   procedure DoOpenFile(Sender: TObject; const AFileName: string);
   procedure BeginOpenFiles(Sender: TObject);
   procedure EndOpenFiles(Sender: TObject);
   procedure ActivateSearchPane(IncrementalSearch: Boolean; SavedSearchFlags: TEdSearchFlags);
   procedure SearchPanelClose;
   procedure SetSearchWrapIndicator(Wrapped: Boolean);
   procedure UpdateIncrSearchText;
   procedure SetActiveEditorControl(Control: TWinControl);
   procedure SetMatchCount(Val: Integer);
   function GetEditorLocalMenu: TMenuItem;
   function EditorViewCount: Integer;
   ...
   class destructor Destroy;
   property EditWindowID: Integer;
   property CanCloneCurEditView: Boolean;
   property Closing: Boolean;
   property CurrentEditor: TWinControl;
   property EditEventHandler: TCustomEditEventHandler;
   property CurrentEditView: TCustomEditView;
   property WasDocked: Boolean;
  ...
  published
   ...
   ModuleMenu          :TPopupActionBar;
   ViewMenu            :TPopupActionBar;
   BottomDockTabSet    :TIDEDockTabSet;
   LeftDockTabSet      :TIDEDockTabSet;
   RightTabDock        :TIDEDockTabSet;
   ModulesEditorPopup  :TPopupMenu;
   dNextPage           :TMenuItem;
   dPrevPage           :TMenuItem;
   ecNextBufferView    :TMenuItem;
   ecPrevBufferView    :TMenuItem;
   ViewToggleFormItem  :TMenuItem;
   ecClosePage         :TMenuItem;
   ecCloseAllOtherPagesItem:TMenuItem;
   ActionList1         :TActionList;
   ecClosePageAction   :TAction;
   ecPagesAction       :TSubMenuAction;
   ecNewEditWindowAction:TAction;
   PanelContainsMiddleStuff:TPanel;
   DockBottomCenterPanel:TEditorDockPanel;
   Panel1              :TPanel;
   GlobalEditorsContainer:TPanel;
   ModuleEditorsContainer:TPanel;
   EditorDockTopSplitter:TSplitter;
   EditorDockTopPanel  :TEditorDockPanel;
   pnlBottomCode       :TPanel;
   ...
   ViewBar             :TTabSet;
   DockBottomMiddleSplitter:TSplitter;
   ecCloseAllOtherPages:TAction;
   TabControl          :TIDEGradientTabSet;
   dNextPageAlt        :TMenuItem;
   dPrevPageAlt        :TMenuItem;
   ecOpenedDuringDebugging:TAction;
   ecCloseFilesOpenedDuringDebugging:TAction;
   SearchPanel         :TPanel;
   SearchTextBox       :THistoryPropComboBox;
   CaseSenseBox        :TPropCheckBox;
   WholeWordBox        :TPropCheckBox;
   RegExBox            :TPropCheckBox;
   WrappedImage        :TImage;
   SearchNotifyPanel   :TPanel;
   WrappedLabel        :TLabel;
   MatchesFoundLabel   :TLabel;
   SearchNavToolbar    :TDockToolBar;
   SearchDnButton      :TToolButton;
   SearchUpButton      :TToolButton;
   SearchWrapAroundButton:TToolButton;
   SearchBarImages     :TImageList;
   SearchSelectionBox  :TPropCheckBox;
   SearchPopupMenu     :TPopupMenu;
   SearchSelectionMenuItem:TMenuItem;
   RegularExpressionMenuItem:TMenuItem;
   WholeWordsMenuItem  :TMenuItem;
   CaseSensitiveMenuItem:TMenuItem;
   SearchBarActions    :TActionList;
   SearchSelectionAction:TAction;
   RegularExpressionAction:TAction;
   WholeWordsAction    :TAction;
   CaseSensitiveAction :TAction;
   SearchPopupSpeedButton:TSpeedButton;
   EntireScopeBox      :TPropCheckBox;
   EntireScopeMenuItem :TMenuItem;
   EntireScopeAction   :TAction;
   procedure ecCloseFilesOpenedDuringDebuggingUpdate(Sender: TObject);
   procedure ecCloseFilesOpenedDuringDebuggingExecute(Sender: TObject);
   procedure ecOpenedDuringDebuggingUpdate(Sender: TObject);
   procedure ecOpenedDuringDebuggingExecute(Sender: TObject);
   procedure TabControlGetOverlayImageIndex(Sender: TObject; TabIndex: Integer; var ImageIndex: Integer);
   procedure TabControlGetHint(Sender: TObject; TabIndex: Integer; var Hint: string);
   procedure ecCloseAllOtherPagesUpdate(Sender: TObject);
   procedure ecCloseAllOtherPagesExecute(Sender: TObject);
   ...
   procedure TabControlStartDrag(Sender: TObject; var DragObject: TDragObject);
   ...
   procedure DockRightPanelDockOver(Sender: TObject; Source: TDragDockObject; X: Integer; Y: Integer; State: TDragState; var Accept: Boolean);
   procedure DockLeftPanelGetSiteInfo(Sender: TObject; DockClient: TControl; var InfluenceRect: TRect; MousePos: TPoint; var CanDock: Boolean);
   procedure FormActivate(Sender: TObject);
   procedure FormShow(Sender: TObject);
   procedure StatusBarDrawPanel(StatusBar: TStatusBar; Panel: TStatusPanel; const Rect: TRect);
   procedure ViewBarChange(Sender: TObject; NewTab: Integer; var AllowChange: Boolean);
   procedure ModuleMenuPopup(Sender: TObject);
   procedure TabControlEndDrag(Sender: TObject; Target: TObject; X: Integer; Y: Integer);
   ...
   procedure dNextPageClick(Sender: TObject);
   procedure dPrevPageClick(Sender: TObject);
   procedure ecPrevBufferViewClick(Sender: TObject);
   procedure ViewToggleFormItemClick(Sender: TObject);
   procedure ecNextBufferViewClick(Sender: TObject);
   procedure ecClosePageClick(Sender: TObject);
   procedure ecNewEditWindowActionExecute(Sender: TObject);
   procedure ecPagesActionPopulateSubMenu(Sender: TObject; AParent: TMenuItem);
   procedure TabControlGetTabHintText(Sender: TObject; const TabIndex: Integer; var HintStr: string; var Handled: Boolean);
   procedure ViewBarClick(Sender: TObject);
   procedure TabControlDblClick(Sender: TObject);
   procedure StatusBarResize(Sender: TObject);
   procedure SearchPanelResize(Sender: TObject);
   procedure SearchTextBoxKeyPress(Sender: TObject; var Key: Char);
   procedure SearchTextBoxExit(Sender: TObject);
   procedure SearchTextBoxEnter(Sender: TObject);
   procedure SearchPanelMouseEnter(Sender: TObject);
   procedure CaseSenseBoxEnter(Sender: TObject);
   procedure WholeWordBoxEnter(Sender: TObject);
   procedure RegExBoxEnter(Sender: TObject);
   procedure SearchUpClick(Sender: TObject);
   procedure SearchDnClick(Sender: TObject);
   procedure SearchRestartClick(Sender: TObject);
   procedure SearchSelectionBoxEnter(Sender: TObject);
   procedure SearchSelectionActionExecute(Sender: TObject);
   procedure RegularExpressionActionExecute(Sender: TObject);
   procedure WholeWordsActionExecute(Sender: TObject);
   procedure CaseSensitiveActionExecute(Sender: TObject);
   procedure SearchPopupSpeedButtonClick(Sender: TObject);
   procedure EntireScopeActionExecute(Sender: TObject);
   procedure EntireScopeBoxEnter(Sender: TObject);
   procedure SearchTextBoxKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
   procedure SearchPanelExit(Sender: TObject);
   procedure FormCreate(Sender: TObject);
   procedure FormDestroy(Sender: TObject);
