...
type
  ...
  TColorizerTabControlStyleHook = class(TColorizerMouseTrackControlStyleHook)
  strict private
    FHotTabIndex: Integer;
    FMousePosition: TColorizerMouseTrackControlStyleHook.TMousePosition;
    FUpDownHandle: HWnd;
...
{ TColorizerTabControlStyleHook }

procedure TColorizerTabControlStyleHook.AngleTextOut(Canvas: TCanvas; Angle, X,
...
procedure TColorizerTabControlStyleHook.CMMouseLeave(var Message: TMessage);
begin
  WMMouseMove(Message);
end;

procedure TColorizerTabControlStyleHook.CNNotify(var Message: TWMNotify);
begin
  if (Message.NMHdr.Code = TCN_SELCHANGE) and (LongWord(Message.IDCtrl) = Handle) and (FUpDownHandle <> 0) then
    UpdateUpDownArea;
end;

constructor TColorizerTabControlStyleHook.Create(AControl: TWinControl);
begin
  inherited;
  DoubleBuffered := True;
  OverridePaint := True;
...
destructor TColorizerTabControlStyleHook.Destroy;
begin
...
procedure TColorizerTabControlStyleHook.DrawTab(Canvas: TCanvas;
  Index: Integer);
var
...
begin
  ...
  begin
    ...
    if ColorizerStyleServices.Available then
      ColorizerStyleServices.DrawIcon(Canvas.Handle, Details, GlyphR, Images.Handle, FImageIndex);
  end;
  ...
  begin
    ...
    begin
      ...
      TY := LayoutR.Top + (LayoutR.Bottom - LayoutR.Top) div 2 -
        Canvas.TextWidth(Tabs[Index]) div 2;
      if ColorizerStyleServices.GetElementColor(Details, ecTextColor, ThemeTextColor)
      then
        Canvas.Font.Color := ThemeTextColor;
      AngleTextOut(Canvas, -900, TX, TY, Tabs[Index]);
    end
...
function TColorizerTabControlStyleHook.GetDisplayRect: TRect;
begin
  Result := Rect(0, 0, 0, 0);
  if (Control <> nil) and (Control is TCustomTabControl) then
    Result := TCustomTabControlClass(Control).DisplayRect
  else
  begin
    Result := Control.ClientRect;
...
function TColorizerTabControlStyleHook.GetImages: TCustomImageList;
begin
  Result := nil;
  if Control is TCustomTabControl then
    Result := TCustomTabControlClass(Control).Images;
end;

function TColorizerTabControlStyleHook.GetScrollOpposite: Boolean;
begin
  Result := False;
  if Control is TCustomTabControl then
    Result := TCustomTabControlClass(Control).ScrollOpposite;
end;

function TColorizerTabControlStyleHook.GetTabCount: Integer;
begin
  Result := SendMessage(Handle, TCM_GETITEMCOUNT, 0, 0);
end;

function TColorizerTabControlStyleHook.GetTabIndex: Integer;
begin
  if Control is TCustomTabControl then
    Result := TCustomTabControlClass(Control).TabIndex
  else
    Result := SendMessage(Handle, TCM_GETCURSEL, 0, 0);
end;


function TColorizerTabControlStyleHook.GetTabPosition: TTabPosition;
begin
  Result := tpTop;
  if Control is TCustomTabControl then
    Result := TCustomTabControlClass(Control).TabPosition;
end;

function TColorizerTabControlStyleHook.GetTabRect(Index: Integer): TRect;
begin
  Result := TRect.Empty;
  if (Control is TCustomTabControl) then
    Result := TCustomTabControlClass(Control).TabRect(Index)
  else if Handle <> 0 then
    TabCtrl_GetItemRect(Handle, Index, Result);
end;

function TColorizerTabControlStyleHook.GetTabs(Index: Integer): string;
var
  TCItem: TTCItem;
...
procedure TColorizerTabControlStyleHook.HookUpDownControl;
begin
...
function TColorizerTabControlStyleHook.IndexOfTabAt(X, Y: Integer): Integer;
var
...
procedure TColorizerTabControlStyleHook.Paint(Canvas: TCanvas);
var
  R: TRect;
  I, SaveIndex: Integer;
  Details: TThemedElementDetails;
begin
  ...
  if ColorizerStyleServices.Available then
  begin
    Details := ColorizerStyleServices.GetElementDetails(ttPane);
    ColorizerStyleServices.DrawElement(Canvas.Handle, Details, R);
  end;
...
procedure TColorizerTabControlStyleHook.PaintBackground(Canvas: TCanvas);
var
  Details: TThemedElementDetails;
begin
  if ColorizerStyleServices.Available then
...
procedure TColorizerTabControlStyleHook.PaintUpDown(Canvas: TCanvas);
var
  Buffer: TBitmap;
  R, BoundsRect: TRect;
...
begin
  ...
  if (BoundsRect.Width = 0) or (BoundsRect.Height = 0) or not ColorizerStyleServices.Available then
    Exit;
  {create buffer}
  Buffer := TBitMap.Create;
  try
...
procedure TColorizerTabControlStyleHook.UpdateTabs(OldHotTab, HotTab: Integer);
var
  R: TRect;
begin
...
procedure TColorizerTabControlStyleHook.UpdateUpDownArea;
var
  R, R1: TRect;
  P: TPoint;
begin
...
procedure TColorizerTabControlStyleHook.UpDownWndProc(var Msg: TMessage);
var
  FCallOldProc: Boolean;
...
procedure TColorizerTabControlStyleHook.WMEraseBkgnd(var Message: TMessage);
var
  Details: TThemedElementDetails;
begin
  if (Message.LParam = 1) and ColorizerStyleServices.Available then
  begin
    Details := ColorizerStyleServices.GetElementDetails(ttPane);
    ColorizerStyleServices.DrawElement(HDC(Message.WParam), Details, Control.ClientRect);
  end;
  Message.Result := 1;
  Handled := True;
end;

procedure TColorizerTabControlStyleHook.WMMouseMove(var Message: TMessage);
var
...
procedure TColorizerTabControlStyleHook.WMParentNotify(var Message: TMessage);
begin
  if (FUpDownHandle = 0) {$IFDEF DELPHIXE3_UP}and (seClient in Control.StyleElements) {$ENDIF} then
    HookUpDownControl;
end;


procedure TColorizerTabControlStyleHook.WndProc(var Message: TMessage);
begin
  // Reserved for potential updates
  inherited;
  case Message.Msg of
